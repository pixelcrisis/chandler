...
module.exports = {
  ...
  run: async function(msg, plugins) {
    ...
    await plugins['zoning'].time(msg, [])
    await plugins['zoning'].time(msg, ['what'])
    await plugins['zoning'].time(msg, ['9pm'])
    await plugins['zoning'].time(msg, ['when', 'ever'])
    await plugins['zoning'].zone(msg, [])
    await plugins['zoning'].zone(msg, ['thing'])
    await plugins['zoning'].zone(msg, ['America/Chicago'])
    await plugins['zoning'].zones(msg, [])
